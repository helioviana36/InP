...
def q4():
    for x in range(0,20):
        nome = input('bota o seu nome:')
        idade = int(input('bota a idade:'))
        sex = input('bota o sexo ai: ')[0].upper()

        if sex == "M" and idade > 21:
            print(f" ola world {nome}" )

#5. Sabendo-se que a unidade lógica e aritmética calcula o produto através de somas
#sucessivas, crie um programa que calcule o produto de dois números inteiros
#lidos. Suponha que os números lidos sejam positivos.
def q5(): 
    print('vai multiplicar do jeito antigo')
    mul = int(input('bota o multiplicado:'))
    x = int(input('bota quantas vezes ele vai se somar:'))
    re = 0
